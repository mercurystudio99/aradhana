...
<div class="sidebar-user">
    <div class='card' style='margin-top:15px;'>
        ...
        <div class="user-profile-avatar" style='background-color:white !important;'>
            <div class="info-new">
                <span class="role-name badge badge-info">{{$dataUser->role_name}}</span>
                <h5 style='color:#10174B;'>{{$dataUser->getDisplayName()}}</h5>
                <p style='color:#10174B'>{{ __("Member Since :time",["time"=> date("M Y",strtotime($dataUser->created_at))]) }}</p>
            </div>
        </div>
    </div>
    <div class="user-profile-plan"  >
        @if( !Auth::user()->hasPermissionTo("dashboard_vendor_access") and setting_item('vendor_enable'))
            <a href=" {{ route("user.upgrade_vendor") }}" style='color:#10174B;'>{{ __("Become a vendor") }}</a>
        @endif
    </div> 
    <div class="sidebar-menu card" style='padding-left:20px;'>
        <ul class="main-menu">
    ...
    <div class="logout card">
        ...
        <a href="#" onclick="event.preventDefault(); document.getElementById('logout-form-vendor').submit();" style='color:#10174B; padding-left:20px' ><i class="fa fa-sign-out" style='color:#10174B'></i> {{__("Log Out")}}
        </a>
        <a href="{{url('/')}}" style="color:#10174B; padding-left:20px;"><i class="fa fa-long-arrow-left" style="color:#10174B;"></i> {{__("Back to Homepage")}}</a>
