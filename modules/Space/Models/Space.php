...
class Space extends Bookable
{
    ...
    public function getMediaGallery($featuredIncluded = false)
    {
        ...
        $items = explode(",", $this->mediagallery);
        foreach ($items as $k => $item) {
            $large = FileHelper::url($item, 'full');
            $thumb = FileHelper::url($item, 'thumb');
            $list_item[] = [
    ...
    public function getDocument($featuredIncluded = false)
    {
        if (empty($this->documents))
            return $this->documents;
        $list_item = [];

        $items = explode(",", $this->documents);
        foreach ($items as $k => $item) {
            if ($item > 0){
                $file=MediaFile::where('id',$item)->first();
                if ($file->file_extension=='pdf') {
                    $large= asset('uploads/' . $file->pdf_path);
                    $thumb= $file->file_name.'.'.$file->file_extension;
                }else{
                    $large=false;
                    $thumb=false;
                }
    ...
    public function getCompareUrl($include_param = true)
    {
        $param = [];
        $user="/property/compare_properties/";
        $urlDetail = app_get_locale(false, false, '/') . $user . $this->id;
        if(!empty($param)){
            $urlDetail .= "?".http_build_query($param);
        }
        return url($urlDetail);
    ...
    public static function getMinMaxSquare()
    {
        $model = parent::selectRaw('MIN( CASE WHEN square > 0 THEN square  END ) AS min_square ,
                                    MAX( CASE WHEN square > 0 THEN square  END ) AS max_square ')->where("status", "publish")->first();
        if (empty($model->min_square) and empty($model->max_square)) {
            return [
                0,
                100
            ];
    ...
    public function hasCompare(){
        return $this->hasOne($this->userCompareClass, 'object_id','id')->where('object_model' , $this->type)->where('user_id' , Auth::id() ?? 0);
    }
    public function isCompare()
    {
        if(Auth::id()){
            if(!empty($this->hasCompare) and !empty($this->hasCompare->id)){
                return 'active';
            }
        }
        return '';
