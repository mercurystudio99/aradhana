    ...
    <form action="{{route('space.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="container-fluid">
            ...
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        @include('Space::admin.space.banner')
                        @include('Space::admin.space.property_detail')
                        @include('Space::admin.space.media')
                        @include('Space::admin.space.calculator')
                        @include('Space::admin.space.facilities')
                        <!-- @include('Space::admin.space.content') -->
                        <!-- @include('Space::admin.space.special-content') -->
                        <!-- @include('Space::admin.space.pricing')
                        @if(is_default_lang())
                            {{--@include('Space::admin.space.availability')--}}
                        @endif -->
                        @include('Space::admin.space.location')
                        <!-- @include('Space::admin.space.surrounding') -->
                                ...
                                <div class="panel-body">
                                    <div class="form-group">
                                        ...
                                        <select name="search_class" class="form-control">
                                            <option @if($row->search_class == 0) selected @endif value=0>{{__("Rent")}}</option>
                                            <option @if($row->search_class == 1) selected @endif value=1>{{__("Sale")}}</option>
                                            <option @if($row->search_class == 2) selected @endif value=2>{{__("Sold")}}</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label>{{__('Property Rera')}}</label>
                                        <br>
                                        <label>
                                            <input type="checkbox" name="is_rera" id="is_rera" onchange="disableRera()" @if($row->is_rera) checked @endif value="1"> {{__("Enable Rera")}}
                                        </label>
                                    </div>
                                    <div class="form-group">
                                        <label>{{__('Rera ID')}}</label>
                                        <br>
                                        <label>
                                            <input type="number" name="rera_id" id="rera_id" @if(!$row->is_rera) disabled @endif class='form-control' value='{{$row->rera_id}}' > 
                                        </label>
                                    </div>
...
@section ('script.body')
    ...
    function disableRera(){
        console.log($("#is_rera")[0].checked);
        if($("#is_rera")[0].checked){
            $("#rera_id")[0].disabled=false;
        }else{
            $("#rera_id")[0].disabled=true;
        }
        console.log($("#rera_id"))
    }
    function openEditNameModal(id,name){
        console.log(id)
        console.log(name)
        $('#documentname').val(name);
        $('#documentid').val(id);
        $("#edit_filename_modal").modal('show');
    }
    function updateFileName(){
        ...
        console.log(id);
        console.log(docname)
        $.post("{{route('space.admin.updatedocuname')}}",
        {
            id:id,
            docname:docname,
            _token: $('#_token').val(),
        ...
        function(data, status){
            $("#edit_filename_modal").modal('hide');
            $('#docfile_name').html(docname)
            location.reload();
            console.log(data);
        }); 
    }
